...
def reloadFPKMHsapiens(fpkmDic, model):
    ...
    for gene in model.genes:
        if not 'G_'+gene.name in fpkmDic:
            continue
...
def setVariables(model, fpkmDic):
    v = vertcat(
    )  # saves the total of variables of the model. Used to """nlp['x']"""
    v_dic = {}
    v_fpkm = {}
    ubx, lbx = [], []
    ##############################################################
    ...
    for i, reaction in enumerate(model.reactions):
        ...
        # useful to discriminate between genes with known fkpm.
        boolean_list = booleanVectorRule(rule, fpkmDic)
        ##########################################################
        # Gets the expression value 'g'
        # get 'g' for reaction with GPR.
...
def createConstraints(model, k, v_dic, sumVi):
    ...
    for met in model.metabolites:
        ...
        constraint = str(met.constraint.expression).replace(
            '+ ', '+').replace('- ', '-')
        ##########################################################
...
def recordTable(record, condition, lbx, ubx, time, status):
    ...
    if record.shape == (0, 0):
        record = pd.DataFrame(
            columns=['Condition', 'N° variables', 'Time', 'Status'])
...
def actuallyTime():
    now = datetime.now()
    actuallyTime = "["+str(now.year)+"/"+str(now.month)+"/"+str(now.day) + \
